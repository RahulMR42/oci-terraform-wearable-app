...
resource "oci_devops_build_pipeline_stage" "adminapi_build_pipeline_stage" {
  ...
  build_pipeline_stage_predecessor_collection {
    ...
    items {
      #Required
      id = oci_devops_build_pipeline.build_pipeline_adminapi.id
    }
  }
  ...
  build_spec_file = var.adminapi_buildspec
...
resource "oci_devops_build_pipeline_stage" "adminapi_deliver_artifact" {

  depends_on = [oci_devops_build_pipeline_stage.adminapi_build_pipeline_stage]
  ...
  build_pipeline_stage_predecessor_collection {
    ...
    items {
      #Required
      id = oci_devops_build_pipeline_stage.adminapi_build_pipeline_stage.id
    }
  }
  ...
  deliver_artifact_collection {
    ...
    #Optional
    items {
      artifact_id   = oci_devops_deploy_artifact.adminapi_service_image.id
...
resource "oci_devops_build_pipeline_stage" "adminapi_invoke_deployment" {

  depends_on = [oci_devops_build_pipeline_stage.adminapi_deliver_artifact]
  ...
  build_pipeline_stage_predecessor_collection {
    ...
    items {
      #Required
      id = oci_devops_build_pipeline_stage.adminapi_deliver_artifact.id
    }
  }
