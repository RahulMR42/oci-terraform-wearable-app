...
resource "oci_devops_build_pipeline_stage" "tcpserver_build_pipeline_stage" {
  #Required
  ...
  build_pipeline_stage_predecessor_collection {
    ...
    items {
      #Required
      id = oci_devops_build_pipeline.build_pipeline_tcpserver.id
    }
  }
  ...
  build_spec_file = var.tcpserver_buildspec
...
resource "oci_devops_build_pipeline_stage" "tcpserver_deliver_artifact" {

  depends_on = [oci_devops_build_pipeline_stage.tcpserver_build_pipeline_stage]
  ...
  build_pipeline_stage_predecessor_collection {
    ...
    items {
      #Required
      id = oci_devops_build_pipeline_stage.tcpserver_build_pipeline_stage.id
    }
  }
  ...
  deliver_artifact_collection {
    ...
    items {
      artifact_id   = oci_devops_deploy_artifact.tcpserver_service_manifest.id
      artifact_name = "tcp_server_k8s_manifest"
    }
    items {
      artifact_id   = oci_devops_deploy_artifact.tcpserver_service_image.id
      artifact_name = "tcp_server_docker_image"
    }
...
resource "oci_devops_build_pipeline_stage" "tcpserver_invoke_deployment" {

  depends_on = [oci_devops_build_pipeline_stage.tcpserver_deliver_artifact]
  ...
  build_pipeline_stage_predecessor_collection {
    ...
    items {
      #Required
      id = oci_devops_build_pipeline_stage.tcpserver_deliver_artifact.id
    }
  }
