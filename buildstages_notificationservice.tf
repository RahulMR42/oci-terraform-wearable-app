...
resource "oci_devops_build_pipeline_stage" "notification_build_pipeline_stage" {
  #Required
  ...
  build_pipeline_stage_predecessor_collection {
    ...
    items {
      #Required
      id = oci_devops_build_pipeline.build_pipeline_notification.id
    }
  }
  ...
  build_spec_file = var.notification_buildspec
...
resource "oci_devops_build_pipeline_stage" "notification_deliver_artifact" {

  depends_on = [oci_devops_build_pipeline_stage.notification_build_pipeline_stage]
  ...
  build_pipeline_stage_predecessor_collection {
    ...
    items {
      #Required
      id = oci_devops_build_pipeline_stage.notification_build_pipeline_stage.id
    }
  }
  ...
  deliver_artifact_collection {
    ...
    #Optional
    items {
      artifact_id   = oci_devops_deploy_artifact.notification_service_image.id
      artifact_name = "notification_service_docker_image"
    }
    items {
      artifact_id   = oci_devops_deploy_artifact.notification_service_manifest.id
      artifact_name = "notification_service_k8s_manifest"
    }
...
resource "oci_devops_build_pipeline_stage" "notificaton_invoke_deployment" {
  ...
  build_pipeline_stage_predecessor_collection {
    ...
    items {
      #Required
      id = oci_devops_build_pipeline_stage.notification_deliver_artifact.id
    }
  }

  build_pipeline_stage_type = var.build_pipeline_stage_deploy_stage_type

  deploy_pipeline_id             = oci_devops_deploy_pipeline.deploy_pipeline_notification.id
